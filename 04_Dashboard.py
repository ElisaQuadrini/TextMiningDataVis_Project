...
DEBERTA_PATH = "Deberta Squad fine tuned"
_deberta_model     = None
...
def answer_question_deberta(question, context):
    ...
    sl = outputs.start_logits[0]
    el = outputs.end_logits[0]
    ...
    s = offsets[best_start][0].item()
    e = offsets[best_end][1].item()
    return context[s:e]
...
# ─────────────────────────────────────────────
BIDAF_PATH = "final_weights_BiDAF"   # path to the BiDAF checkpoint (e.g. ~/Desktop/BiDAF_finale)
VOCAB_PATH = "bidaf_vocab.pkl"
...
class BiDAFQA(nn.Module):
    ...
    def __init__(self, vocab_size, char_vocab_size, embed_dim=300, char_embed_dim=64,
        ...
        self.self_attn_q    = nn.Linear(H, H, bias=False)
        self.self_attn_k    = nn.Linear(H, H, bias=False)
        self.self_attn_out  = nn.Linear(H, H)
        self.self_attn_norm = nn.LayerNorm(H)

        self.start_head = nn.Linear(H * 2, 1)

        self.end_lstm = nn.LSTM(input_size=H, hidden_size=hidden_dim,
    ...
    def _highway(self, x):
        for layer in self.highway_layers:
            combined  = layer(x)
            transform = torch.relu(combined[..., :x.size(-1)])
            gate      = torch.sigmoid(combined[..., x.size(-1):])
    ...
    def _bidaf_attention(self, ctx, q, ctx_mask, q_mask):
        B, Lc, H = ctx.size()
        Lq = q.size(1)
        ctx_exp = ctx.unsqueeze(2).expand(B, Lc, Lq, H)
        q_exp   = q.unsqueeze(1).expand(B, Lc, Lq, H)
        S = self.attn_w(torch.cat([ctx_exp, q_exp, ctx_exp * q_exp], dim=-1)).squeeze(-1)
        S = S.masked_fill(q_mask.unsqueeze(1).expand(B, Lc, Lq) == 0, -1e9)
        c2q = torch.bmm(torch.softmax(S, dim=2), q)
        S2  = S.masked_fill(ctx_mask.unsqueeze(2).expand(B, Lc, Lq) == 0, -1e9)
        q2c = torch.bmm(torch.softmax(S2.max(dim=2).values, dim=1).unsqueeze(1), ctx).expand(B, Lc, H)
        return c2q, q2c
    ...
    def _self_attention(self, x, mask):
        ...
        scores = scores.masked_fill(mask.unsqueeze(1).expand(B, L, L) == 0, -1e9)
    ...
    def forward(self, ctx_ids, ctx_masks, ctx_chars, ctx_em, q_ids, q_masks, q_chars):
        ctx_word = self.dropout(self.word_embedding(ctx_ids))
        q_word   = self.dropout(self.word_embedding(q_ids))

        ctx_char_enc = self.dropout(self.char_encoder(ctx_chars))
        q_char_enc   = self.dropout(self.char_encoder(q_chars))

        ctx_em_f = ctx_em.unsqueeze(-1)
        q_em_f   = torch.zeros(q_word.size(0), q_word.size(1), 1, device=q_word.device)

        ctx_emb = torch.cat([ctx_word, ctx_char_enc, ctx_em_f], dim=-1)
        q_emb   = torch.cat([q_word,   q_char_enc,   q_em_f],   dim=-1)

        ctx_emb = self._highway(torch.relu(self.highway_proj(ctx_emb)))
        q_emb   = self._highway(torch.relu(self.highway_proj(q_emb)))

        ctx_enc, _ = self.ctx_lstm(ctx_emb)
        ...
        modeled, _ = self.model_lstm(fused_proj)
        modeled    = self.dropout(modeled)

        modeled_sa = self._self_attention(modeled, ctx_masks)

        start_repr   = torch.cat([fused_proj, modeled_sa], dim=-1)
        start_logits = self.start_head(start_repr).squeeze(-1)

        end_enc, _  = self.end_lstm(modeled_sa)
        end_repr    = torch.cat([fused_proj, end_enc], dim=-1)
        end_logits  = self.end_head(end_repr).squeeze(-1)

        start_logits = start_logits.masked_fill(ctx_masks == 0, -1e9)
        end_logits   = end_logits.masked_fill(ctx_masks   == 0, -1e9)

        start_pool = self._masked_pool(start_repr, ctx_masks)
        q_pool     = self._masked_pool(q_enc, q_masks)
...
def get_bidaf():
    ...
    print("BIDAF_PATH:", BIDAF_PATH)
    print("BIDAF exists:", Path(BIDAF_PATH).exists())
    print("VOCAB_PATH:", VOCAB_PATH)
    print("VOCAB exists:", Path(VOCAB_PATH).exists())

    _bidaf_device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # carica checkpoint pesi
    ckpt = torch.load(BIDAF_PATH, map_location=_bidaf_device, weights_only=False)
    print("Checkpoint keys:", ckpt.keys())

    # carica vocabolari dal file pickle
    ...
    if isinstance(vocab, dict):
        ...
        if "word2idx" in vocab and "char2idx" in vocab:
            _bidaf_word2idx = vocab["word2idx"]
            _bidaf_char2idx = vocab["char2idx"]
        else:
            raise ValueError(
                f"{VOCAB_PATH} è un dict ma non contiene 'word2idx' e 'char2idx'. "
                f"Chiavi trovate: {list(vocab.keys())}"
            )

    elif isinstance(vocab, tuple) and len(vocab) == 2:
        _bidaf_word2idx, _bidaf_char2idx = vocab

    else:
        raise ValueError(
            f"Formato vocab non supportato in {VOCAB_PATH}. Tipo trovato: {type(vocab)}"
        )

    print("Word vocab size:", len(_bidaf_word2idx))
    print("Char vocab size:", len(_bidaf_char2idx))
    ...
    _bidaf_model = BiDAFQA(
        ...
        char_embed_dim=_CHAR_EMBED_DIM,
        char_out_dim=_CHAR_OUT_DIM,
        char_kernel=_CHAR_KERNEL,
        hidden_dim=_HIDDEN_DIM,
        dropout=_DROPOUT,
        num_highway=_NUM_HIGHWAY,
    ).to(_bidaf_device)
...
def fig_confusion_matrix(cm, labels, title, accent_color):
    """Heatmap confusion matrix using Plotly."""
    # cm = [[TN, FP], [FN, TP]]
    z    = cm
    ...
    fig  = go.Figure(go.Heatmap(
        ...
        colorscale=[[0, "#EFF6FF"], [1, accent_color]],
        showscale=False,
    ...
    fig.update_layout(
        **LAYOUT_BASE,
        title=title,
        xaxis=dict(title="Predicted label", tickfont=dict(size=12)),
...
def fig_dep_relations(df):
    """Top-15 dependency relations in questions (overall)."""
    import spacy as _spacy
    try:
        _nlp = _spacy.load("en_core_web_sm")
    except Exception:
        fig = go.Figure()
        fig.update_layout(**LAYOUT_BASE, title="spaCy model not found")
        return fig
    SAMPLE = min(3000, len(df))
    texts  = df["question"].sample(SAMPLE, random_state=42).tolist()
    docs   = list(_nlp.pipe(texts, batch_size=128))
    from collections import Counter as _Ctr
    dep_counts = _Ctr([tok.dep_ for doc in docs for tok in doc if tok.dep_ != "punct"])
    top = dep_counts.most_common(15)
    ...
    fig = go.Figure(go.Bar(
        ...
        marker_color="#0891B2", marker_line_width=0,
        text=[f"{v/total:.1%}" for v in vals],
        textposition="outside",
    ))
    fig.update_layout(
        **LAYOUT_BASE,
        title="Top 15 Dependency Relations in Questions",
        xaxis=dict(title="Count", gridcolor=COLORS["grid"]),
        height=max(320, 15 * 28),
    )
    return fig


def fig_dep_by_answerability(df):
    """Dependency relations by answerability (answerable vs unanswerable)."""
    import spacy as _spacy
    from collections import Counter as _Ctr
    try:
        _nlp = _spacy.load("en_core_web_sm")
    ...
    docs = list(_nlp.pipe(sample_df["question"].tolist(), batch_size=128))
    dep_ans, dep_unans = _Ctr(), _Ctr()
    for doc, y in zip(docs, sample_df["y"]):
        deps = [tok.dep_ for tok in doc if tok.dep_ not in ("punct", "ROOT")]
        (dep_ans if y == 1 else dep_unans).update(deps)
    total_a, total_u = sum(dep_ans.values()), sum(dep_unans.values())
    top_k   = 12
    all_dep = list({k for k,_ in dep_ans.most_common(top_k)} | {k for k,_ in dep_unans.most_common(top_k)})
    all_dep = sorted(all_dep, key=lambda d: (dep_ans.get(d,0)/total_a + dep_unans.get(d,0)/total_u)/2, reverse=True)
    perc_a = [dep_ans.get(d,0)/total_a*100 for d in all_dep]
    perc_u = [dep_unans.get(d,0)/total_u*100 for d in all_dep]
    fig = go.Figure([
        go.Bar(name="Answerable",   x=all_dep, y=perc_a, marker_color=COLORS["answerable"], opacity=0.88),
        go.Bar(name="Unanswerable", x=all_dep, y=perc_u, marker_color="#F59E0B",            opacity=0.88),
    ])
    fig.update_layout(
        **LAYOUT_BASE, barmode="group",
        title="Dependency Relations by Answerability",
        xaxis=dict(tickangle=35),
        yaxis=dict(title="Percentage (%)", gridcolor=COLORS["grid"]),
...
def fig_root_pos(df):
    """POS tag of the ROOT token in questions."""
    import spacy as _spacy
    from collections import Counter as _Ctr
    try:
        _nlp = _spacy.load("en_core_web_sm")
    ...
    fig.update_layout(
        **LAYOUT_BASE,
        title="POS Tag of ROOT Token in Questions",
        xaxis=dict(title="Count", gridcolor=COLORS["grid"]),
        height=max(260, len(items) * 40),
    )
    return fig
...
@app.callback(
    ...
    Output("sidebar-dependency",  "className"),
    Output("sidebar-wordclouds",  "className"),
    [Input(f"sidebar-{k}", "n_clicks") for k in
     ["overview", "lengths", "wh", "tokens", "topics", "dependency", "wordclouds"]],
    State("eda-section-local", "data"),
    prevent_initial_call=False,
)
def switch_eda_section(*args):
    from dash import ctx
    keys    = ["overview", "lengths", "wh", "tokens", "topics", "dependency", "wordclouds"]
...
def render_eda_section(section):
    ...
    if section == "overview" or section is None:
        ...
        selector = html.Div(className="group-selector", children=[
            html.Span("Select chart", className="group-selector-label"),
            dcc.Dropdown(
                id="dep-mode",
                options=[
                    {"label": "Top 15 dependency relations",  "value": "overall"},
                    {"label": "Relations by answerability",   "value": "answerability"},
                    {"label": "POS tag of ROOT token",        "value": "root"},
                ],
                value="overall", clearable=False,
                style={"minWidth": "280px", "fontFamily": "DM Sans, sans-serif"},
            ),
        ])
        return html.Div([
            selector,
            html.Div(id="dep-chart-container", children=[
                graph_card("Top 15 Dependency Relations in Questions",
                           fig_dep_relations(df), height="480px"),
            ]),
        ])
...
@app.callback(
    Output("dep-chart-container", "children"),
    Input("dep-mode", "value"),
    prevent_initial_call=True,
)
def update_dep_chart(mode):
    if mode == "overall":
        return graph_card("Top 15 Dependency Relations in Questions",
                          fig_dep_relations(train_df), height="480px")
    elif mode == "answerability":
        return graph_card("Dependency Relations by Answerability",
                          fig_dep_by_answerability(train_df), height="420px")
    else:
        return graph_card("POS Tag of ROOT Token in Questions",
                          fig_root_pos(train_df), height="360px")
...
def render_bidaf_section(section):
    ...
    if section == "overview" or section is None:
        ...
        # BiDAF test confusion matrix: [[TN=3466, FP=295], [FN=2385, TP=2844]]
        cm_bidaf = [[3466, 295], [2385, 2844]]
        row3 = dbc.Row([
            dbc.Col(graph_card(
                "Confusion Matrix — Test set (threshold=0.80)",
                fig_confusion_matrix(cm_bidaf, ["Unanswerable", "Answerable"],
                                     "Confusion Matrix — Test set (threshold=0.80)", "#7C3AED"),
                height="360px",
            ), md=6),
        ])
...
def render_deberta_section(section):
    ...
    if section == "overview" or section is None:
        ...
        # DeBERTa test confusion matrix: [[TN=3380, FP=418], [FN=348, TP=4892]]
        cm_deberta = [[3380, 418], [348, 4892]]
        row3 = dbc.Row([
            dbc.Col(graph_card(
                "Confusion Matrix — Test set",
                fig_confusion_matrix(cm_deberta, ["Unanswerable", "Answerable"],
                                     "Confusion Matrix — Test set", "#0891B2"),
                height="360px",
            ), md=6),
        ])
        return html.Div([kpi_row, row1, row2, row3])
        ...
        return html.Div([
            ...
            html.Div(className="chart-card", style={"padding": "0"}, children=[
                ...
                html.Div(style={"padding": "24px"}, children=[
                    html.P("Type any question. The retriever returns the top-5 most relevant contexts "
